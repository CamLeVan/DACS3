...
class AuthViewModel @Inject constructor(
    ...
    fun register(name: String, email: String, password: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                Log.e(TAG, "Lỗi không mong đợi trong quá trình đăng ký", e)
                _uiState.value = AuthUiState.Error("Lỗi không mong đợi: ${e.message}")
            }
        }
    }
    ...
    fun loginWithGoogle(idToken: String? = null) {
        viewModelScope.launch {
            ...
            if (idToken == null) {
                // Đây chỉ là một placeholder cho UI
                // Luồng đăng nhập Google thực tế sẽ được xử lý bởi Activity
                Log.d(TAG, "Đang yêu cầu đăng nhập Google từ Activity")
            ...
            try {
                ...
                result.fold(
                    ...
                    onFailure = { error ->
                        Log.e(TAG, "Lỗi đăng nhập Google", error)
                        _uiState.value = AuthUiState.Error(error.message ?: "Đăng nhập Google thất bại")
                    }
                )
            } catch (e: Exception) {
                Log.e(TAG, "Ngoại lệ đăng nhập Google", e)
                _uiState.value = AuthUiState.Error("Đăng nhập Google thất bại: ${e.message}")
            }
        }
    }
...
sealed class AuthUiState {
    object Idle : AuthUiState() // Trạng thái chờ
    object Loading : AuthUiState() // Đang tải
    data class Success(val user: User) : AuthUiState() // Thành công
    data class Error(val message: String) : AuthUiState() // Lỗi
}

sealed class AuthEvent {
    object LoginSuccess : AuthEvent() // Đăng nhập thành công
    object RegisterSuccess : AuthEvent() // Đăng ký thành công
    object GoogleSignInRequested : AuthEvent() // Yêu cầu đăng nhập Google
    object BiometricAuthRequested : AuthEvent() // Yêu cầu xác thực sinh trắc học
}
