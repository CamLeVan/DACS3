...
import com.example.taskapplication.data.worker.SyncWorker as DataSyncWorker
...
class SyncRepositoryImpl @Inject constructor(
    ...
    override suspend fun schedulePeriodicSync(intervalMinutes: Int) {
        ...
        try {
            // Thử sử dụng SyncWorker từ package workers
            SyncWorker.schedulePeriodicSync(context, intervalMinutes)
            Log.d(TAG, "Scheduled periodic sync using workers.SyncWorker")
        } catch (e: Exception) {
            Log.e(TAG, "Error scheduling with workers.SyncWorker, trying data.worker.SyncWorker", e)
            // Fallback to data.worker.SyncWorker
            DataSyncWorker.schedulePeriodicSync(context, intervalMinutes)
            Log.d(TAG, "Scheduled periodic sync using data.worker.SyncWorker")
        }
    }
    ...
    override suspend fun cancelPeriodicSync() {
        try {
            ...
            Log.d(TAG, "Cancelled periodic sync using workers.SyncWorker")
        } catch (e: Exception) {
            Log.e(TAG, "Error cancelling with workers.SyncWorker, trying data.worker.SyncWorker", e)
            // Fallback to data.worker.SyncWorker
            DataSyncWorker.cancelPeriodicSync(context)
            Log.d(TAG, "Cancelled periodic sync using data.worker.SyncWorker")
        }
    }
