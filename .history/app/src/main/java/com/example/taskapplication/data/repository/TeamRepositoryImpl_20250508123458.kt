...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<TeamMember> {
        ...
        return try {
            val response = apiService.inviteUserToTeam(teamId, AddTeamMemberRequest(userEmail))
            if (response.isSuccessful) {
                val member = response.body()?.toEntity()
                if (member != null) {
                    teamMemberDao.insertMember(member)
                    Result.success(member.toDomainModel())
                } else {
                    Result.failure(IllegalStateException("Failed to create team member"))
                }
            } else {
                Result.failure(IllegalStateException("Failed to invite user to team"))
            }
        } catch (e: Exception) {
    ...
    override suspend fun changeUserRole(teamId: String, userId: String, newRole: String): Result<TeamMember> {
        ...
        if (existingTeam.createdBy != currentUserId) {
            return Result.failure(IllegalStateException("Not authorized to change user roles in this team"))
        }

        val existingMember = teamMemberDao.getTeamMember(teamId, userId) ?:
            return Result.failure(IllegalStateException("Team member not found"))
        ...
        return try {
            teamMemberDao.updateMember(updatedMember)
    ...
    override fun isUserAdminOfTeam(teamId: String, userId: String): Flow<Boolean> {
        return teamMemberDao.getTeamMember(teamId, userId).map { member ->
            member?.role == "admin"
        }
    }
