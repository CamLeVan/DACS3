...
class ChatMessageRepositoryImpl @Inject constructor(
                ...
                try {
                    ...
                    if (attachments != null && attachments.isNotEmpty()) {
                        for (attachment in attachments) {
                            ...
                            if (file.exists()) {
                                val mediaType = MediaType.parse(attachment.fileType) ?: MediaType.parse("application/octet-stream")
                                val requestFile = RequestBody.create(mediaType, file)
                                val filePart = MultipartBody.Part.createFormData("file", file.name, requestFile)
                                val textPlainMediaType = MediaType.parse("text/plain") ?: MediaType.parse("text/plain")
                                val typeBody = RequestBody.create(textPlainMediaType, "attachment")
    ...
    override suspend fun markMessageAsRead(messageId: String): Result<Unit> {
        try {
            ...
            val checkReadStatusId = "${messageId}_${currentUserId}"
            val existingReadStatus = messageReadStatusDao.getReadStatus(checkReadStatusId)
    ...
    override suspend fun addReaction(messageId: String, reaction: String): Result<MessageReaction> {
        try {
            ...
            val reactionEntity = com.example.taskapplication.data.database.entities.MessageReactionEntity(
                id = reactionId,
                messageId = messageId,
                userId = currentUserId,
                reaction = reaction,
    ...
    override suspend fun removeReaction(messageId: String, reaction: String): Result<Unit> {
        try {
            ...
            // Xóa reaction
            val reactionId = "${messageId}_${currentUserId}_${reaction}"
                ...
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (unreadCounts != null) {
                            unreadCounts.data.forEach { entry ->
                                val teamId = entry.key.toString()
