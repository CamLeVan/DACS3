...
class PersonalTaskRepositoryImpl @Inject constructor(
    ...
    override suspend fun syncTasks(): Result<Unit> {
        return try {
            ...
            // Sync pending tasks
            val pendingTasks = personalTaskDao.getPendingTasksSync()
            ...
            if (response.isSuccessful) {
                response.body()?.let { tasks ->
                    for (taskResponse in tasks) {
                        val existingTask = personalTaskDao.getTaskByServerIdSync(taskResponse.id)
                        val entity = taskResponse.toEntity(existingTask)
                ...
                Result.failure(Exception("Failed to sync tasks: ${response.code()}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
