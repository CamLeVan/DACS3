...
class BiometricVerifyUseCase @Inject constructor(
    ...
    suspend operator fun invoke(signature: String): Result<User> {
        try {
            ...
            // Biometric đã bị loại bỏ khỏi API, sử dụng login thông thường
            return authRepository.getCurrentUser() ?: Result.failure(Exception("Không thể lấy thông tin người dùng hiện tại"))
        } catch (e: Exception) {
