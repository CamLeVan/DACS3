                ...
                try {
                    ...
                    if (attachments != null && attachments.isNotEmpty()) {
                        for (attachment in attachments) {
                            ...
                            if (file.exists()) {
                                // Sử dụng cách khác để tạo MediaType
                                val requestFile = RequestBody.create(MediaType.parse(attachment.fileType) ?: MediaType.parse("application/octet-stream")!!, file)
                                val filePart = MultipartBody.Part.createFormData("file", file.name, requestFile)
                                val typeBody = RequestBody.create(MediaType.parse("text/plain")!!, "attachment")
