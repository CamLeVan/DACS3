...
class PersonalTaskRepositoryImpl @Inject constructor(
    ...
    override suspend fun getTaskCategories(): Result<List<String>> {
        return try {
            if (!connectionChecker.isNetworkAvailable()) {
                return Result.failure(IOException("No network connection"))
            }

            val response = apiService.getPersonalTaskCategories()
            if (response.isSuccessful && response.body() != null) {
                Result.success(response.body()!!)
            } else {
                val errorBody = response.errorBody()?.string() ?: "Unknown error"
                Log.e(TAG, "Error getting task categories: $errorBody")
                Result.failure(Exception(errorBody))
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error getting task categories", e)
            Result.failure(e)
        }
    }

    override suspend fun getTaskStatistics(period: String?, startDate: Long?, endDate: Long?): Result<Map<String, Any>> {
        return try {
            if (!connectionChecker.isNetworkAvailable()) {
                return Result.failure(IOException("No network connection"))
            }
            ...
            val response = apiService.getPersonalTaskStatistics(period, startDateStr, endDateStr)
            if (response.isSuccessful && response.body() != null) {
                Result.success(response.body()!!)
            } else {
                val errorBody = response.errorBody()?.string() ?: "Unknown error"
                Log.e(TAG, "Error getting task statistics: $errorBody")
                Result.failure(Exception(errorBody))
            }
        } catch (e: Exception) {
    ...
    override suspend fun getTasksFromServer(
        status: String?,
        priority: String?,
        category: String?,
        dueDateStart: Long?,
        ...
        return try {
            ...
            val dueDateStartStr = dueDateStart?.let { Instant.ofEpochMilli(it).toString() }
            val dueDateEndStr = dueDateEnd?.let { Instant.ofEpochMilli(it).toString() }
