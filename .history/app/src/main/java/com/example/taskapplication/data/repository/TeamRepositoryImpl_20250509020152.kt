...
class TeamRepositoryImpl @Inject constructor(
    ...
    override fun getTeamsByOwner(ownerId: String): Flow<List<Team>> {
        return teamDao.getTeamsByOwner(ownerId)
            .map { entities -> entities.map { it.toDomainModel() } }
            .flowOn(Dispatchers.IO)
    }
    ...
    override suspend fun getTeamMembers(teamId: String): Result<List<User>> {
        try {
            val members = teamMemberDao.getTeamMembers(teamId).collect { memberEntities ->
                val users = memberEntities.mapNotNull { member ->
                    userDao.getUserById(member.userId)?.toDomainModel()
                }
    ...
    override suspend fun addTeamMember(teamId: String, userId: String, role: String): Result<Unit> {
        try {
            // Kiểm tra xem team và user có tồn tại không
            val team = teamDao.getTeamByIdSync(teamId)
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    Log.e(TAG, "Error syncing team member addition to server", e)
                    // Không trả về lỗi vì đã lưu thành công vào local database
                }
            }
