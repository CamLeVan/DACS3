...
class PersonalTasksViewModel @Inject constructor(
    ...
    fun createTask(task: PersonalTask) {
        viewModelScope.launch {
            ...
            val taskWithOrder = task.copy(order = maxOrder + 1)
    ...
    fun updateTasksOrder(taskOrders: Map<String, Int>) {
        viewModelScope.launch {
            personalTaskRepository.updateTasksOrder(taskOrders)
        }
    }
    ...
    fun createSubtask(taskId: String, title: String) {
        viewModelScope.launch {
            val subtask = Subtask(
                ...
                title = title,
                completed = false,
                order = 0, // Sẽ được cập nhật thành max+1
