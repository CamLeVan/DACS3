...
class UserRepositoryImpl @Inject constructor(
    ...
    override suspend fun searchUsers(query: String): List<User> {
        try {
            ...
            val localResults = userDao.searchUsers(query).map { it.toDomainModel() }
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        // Kết hợp kết quả từ local và server, loại bỏ trùng lặp
                        val combinedResults = (localResults + serverResults).distinctBy { it.id }
