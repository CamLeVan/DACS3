...
class TeamTaskRepositoryImpl @Inject constructor(
    ...
    override fun getTaskById(taskId: String): Flow<TeamTask?> {
        return teamTaskDao.getTaskById(taskId)
    ...
    override suspend fun updateTask(task: TeamTask): Result<TeamTask> {
        return try {
            if (networkUtils.isNetworkAvailable()) {
                val response = apiService.updateTeamTask(task.toEntity().toApiRequest())
                if (response.isSuccessful) {
                    response.body()?.let { taskResponse ->
                        val entity = taskResponse.toEntity()
        ...
        return try {
            ...
            for (task in pendingTasks) {
                when (task.syncStatus) {
                    ...
                    "pending_update" -> {
                        val response = apiService.updateTeamTask(task.toApiRequest())
                        if (response.isSuccessful) {
                            response.body()?.let { taskResponse ->
                                val entity = taskResponse.toEntity()
                                teamTaskDao.updateTask(entity)
