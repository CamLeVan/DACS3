    ...
    override suspend fun sendMessage(message: Message): Result<Message> {
        return try {
            if (!networkUtils.isNetworkAvailable()) {
                ...
                val request = message.toEntity().toApiRequest()
    ...
    override suspend fun updateMessage(message: Message): Result<Message> {
        return try {
            if (!networkUtils.isNetworkAvailable()) {
                ...
                val request = message.toEntity().toApiRequest()
                val response = apiService.updateMessage(message.id, request)
                val entity = response.toEntity()
                messageDao.updateMessage(entity)
                Result.success(entity.toDomainModel())
            }
    ...
    override suspend fun markMessageAsRead(messageId: Long, userId: String): Result<Unit> {
        return try {
            if (!networkUtils.isNetworkAvailable()) {
                ...
                apiService.markMessageAsRead(messageId, userId)
                messageDao.markMessageAsRead(messageId, userId)
                Result.success(Unit)
            }
        } catch (e: Exception) {
            ...
            if (!networkUtils.isNetworkAvailable()) {
                ...
                for (message in pendingMessages) {
                    when (message.syncStatus) {
                        "pending_create" -> {
                            val request = message.toApiRequest()
                            val response = apiService.sendMessage(request)
                            val entity = response.toEntity()
                            messageDao.insertMessage(entity)
                        }
                ...
                for (messageResponse in serverMessages) {
    ...
    override suspend fun syncTeamMessages(teamId: String): Result<Unit> {
        return try {
            if (!networkUtils.isNetworkAvailable()) {
                ...
                val serverMessages = apiService.getTeamMessages(teamId)
                for (messageResponse in serverMessages) {
                    val entity = messageResponse.toEntity()
                    messageDao.insertMessage(entity)
                }
