    ...
    companion object {
        ...
        val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                database.execSQL(
                    """
                    CREATE TABLE IF NOT EXISTS `team_documents_temp` (
                        `id` TEXT PRIMARY KEY NOT NULL,
                        `teamId` TEXT NOT NULL,
                        `name` TEXT NOT NULL,
                        `description` TEXT NOT NULL,
                ...
                database.execSQL(
                    """
                    INSERT OR IGNORE INTO `team_documents_temp`
                    SELECT id, teamId, name, description, fileUrl, fileType, fileSize, uploadedBy,
                           uploadedAt, lastModified, serverId, syncStatus, accessLevel, allowedUsers,
                           CASE WHEN isDeleted THEN 1 ELSE 0 END as isDeleted
                    FROM `team_documents`
                    """
                )
                ...
                // Đổi tên bảng tạm thời thành tên bảng chính
                database.execSQL("ALTER TABLE `team_documents_temp` RENAME TO `team_documents`")
