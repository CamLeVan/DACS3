...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<TeamMember> {
        ...
        try {
            val response = apiService.findUserByEmail(userEmail)
        ...
        try {
            ...
            for (member in membersToCreate) {
                try {
                    ...
                    val memberRequest = member.toApiRequest().copy(teamId = team.serverId)
                    val response = apiService.addTeamMember(memberRequest)
            ...
            for (member in membersToUpdate) {
                try {
                    ...
                    // Use server id for the team
                    val memberRequest = member.toApiRequest().copy(teamId = team.serverId)
                    val response = apiService.updateTeamMember(member.serverId, memberRequest)
            ...
            for (member in membersToDelete) {
                ...
                try {
                    val response = apiService.removeTeamMember(member.serverId)
