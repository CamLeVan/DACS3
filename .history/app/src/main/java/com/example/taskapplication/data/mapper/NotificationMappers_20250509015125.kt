...
fun NotificationSettings.toApiRequest(): NotificationSettingsRequest {
    return NotificationSettingsRequest(
        ...
        quiet_hours = mapOf(
            "enabled" to quietHoursEnabled.toString(),
            "start" to quietHoursStart,
            "end" to quietHoursEnd
        )
    )
}
...
fun NotificationSettingsResponse.toDomainModel(): NotificationSettings {
    return NotificationSettings(
        ...
        quietHoursEnabled = quiet_hours?.get("enabled")?.toBoolean() ?: false,
        quietHoursStart = quiet_hours?.get("start") ?: "22:00",
        quietHoursEnd = quiet_hours?.get("end") ?: "07:00"
    )
}
