...
class MessageRepositoryImpl @Inject constructor(
        ...
        try {
            ...
            for (message in messagesToUpdate) {
                try {
                    ...
                    // Use server id for the team
                    val messageRequest = message.toApiRequest()
                    val response = apiService.updateMessage(team.serverId!!.toLong(), message.serverId!!.toLong(), messageRequest)
            ...
            for (message in messagesToDelete) {
                ...
                try {
                    // Find the team for this message
                    val team = teamDao.getTeamByIdSync(message.teamId ?: "")
                    if (team == null || team.serverId == null) {
                        continue // Skip if team not found or not synced
                    }

                    val response = apiService.deleteMessage(team.serverId!!.toLong(), message.serverId!!.toLong())
            ...
            try {
                ...
                // For each team, get messages
                for (team in teams) {
                    if (team.serverId == null) continue

                    val response = apiService.getTeamChat(team.serverId!!.toLong(), lastSyncTimestamp)

                    if (response.isSuccessful && response.body() != null) {
                        val remoteMessages = response.body()!!

                        for (remoteMessage in remoteMessages) {
                            // We already have the local team
                            val localMessage = messageDao.getMessageByServerIdSync(remoteMessage.id.toString())
                            ...
                            if (localMessage == null) {
                                ...
                                messageDao.insertMessage(
                                    remoteMessage.toEntity().copy(
                                        id = UUID.randomUUID().toString(),
                                        teamId = team.id, // Use local team ID
                                        syncStatus = "synced"
                                    )
                                )
                            } else if (remoteMessage.timestamp > localMessage.lastModified &&
    ...
    override suspend fun markMessageAsDeleted(messageId: Long) {
        messageDao.markMessageAsDeleted(messageId.toString(), System.currentTimeMillis())
    }
}