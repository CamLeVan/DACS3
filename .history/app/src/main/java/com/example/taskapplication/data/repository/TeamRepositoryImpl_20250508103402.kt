...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<TeamMember> {
        ...
        try {
            ...
            val response = apiService.addTeamMember(teamId, request)
            ...
            for (team in teamsToCreate) {
                try {
                    ...
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        teamDao.updateTeamServerId(team.id, serverTeam.id)
        ...
        try {
            ...
            for (member in membersToCreate) {
                try {
                    ...
                    val response = apiService.addTeamMember(member.teamId, request)
                    if (response.isSuccessful && response.body() != null) {
                        val serverMember = response.body()!!
                        teamMemberDao.updateTeamMemberServerId(member.id, serverMember.id)
            ...
            for (member in membersToUpdate) {
                try {
                    ...
                    val response = apiService.updateTeamMember(member.teamId, member.serverId!!, request)
                    if (response.isSuccessful) {
                        teamMemberDao.markTeamMemberAsSynced(member.id)
                    }
                } catch (e: Exception) {
