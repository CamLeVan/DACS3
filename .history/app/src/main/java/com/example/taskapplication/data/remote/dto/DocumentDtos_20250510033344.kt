...
fun DocumentVersionDto.toDomain(): DocumentVersion {
    return DocumentVersion(
        ...
        fileSize = fileSize,
        uploadedBy = uploadedBy,
        uploadedAt = parseApiDate(uploadedAt),
...
fun DocumentPermissionDto.toDomain(): DocumentPermission {
    return DocumentPermission(
        ...
        grantedBy = grantedBy,
        grantedAt = parseApiDate(grantedAt),
        syncStatus = "synced",
        serverId = id
    )
}
...
/**
 * Helper function to parse API date string to Date
 */
private fun parseApiDate(dateString: String): Date {
    ...
    return try {
        ...
        java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'").parse(dateString) ?: Date()
    } catch (e: Exception) {
        Date()
    }
}
