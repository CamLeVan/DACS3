...
interface MessageReactionDao {
    ...
    @Query("UPDATE message_reactions SET syncStatus = 'synced' WHERE id = :reactionId")
    suspend fun markReactionAsSynced(reactionId: String)
    ...
    @Query("SELECT * FROM message_reactions WHERE syncStatus = 'pending'")
    suspend fun getPendingReactions(): List<MessageReactionEntity>

    @Query("UPDATE message_reactions SET syncStatus = 'synced', serverId = :serverId, lastModified = :timestamp WHERE id = :id")
