...
import com.example.taskapplication.data.util.DeviceInfoProvider
...
class PersonalTaskRepositoryImpl @Inject constructor(
    ...
    override suspend fun searchTasksFromServer(
        ...
        return try {
            ...
            val response = apiService.searchPersonalTasks(
                query = query,
                page = page,
                perPage = perPage
            )
    ...
    override fun getSubtasks(taskId: String): Flow<List<Subtask>> {
        return personalTaskDao.getSubtasksByTaskId(taskId)
            .map { entities -> entities.map { it.toDomainModel() } }
            .flowOn(Dispatchers.IO)
    }

    override suspend fun getSubtask(subtaskId: String): Subtask? {
        return personalTaskDao.getSubtaskById(subtaskId)?.toDomainModel()
    }

    override suspend fun createSubtask(taskId: String, subtask: Subtask): Result<Subtask> {
        try {
            // Lưu vào local database trước với trạng thái pending_create
            val subtaskEntity = subtask.toEntity().copy(
                syncStatus = PersonalTask.SyncStatus.CREATED,
                lastModified = System.currentTimeMillis()
            )
            personalTaskDao.insertSubtask(subtaskEntity)

            // Nếu có kết nối mạng và task đã có serverId, đồng bộ lên server
            val task = personalTaskDao.getTaskById(taskId)
            if (connectionChecker.isNetworkAvailable() && task?.serverId != null) {
                try {
                    // Gọi API để tạo subtask trên server
                    val request = subtask.toApiRequest()
                    val response = apiService.createSubtask(task.serverId, request)

                    if (response.isSuccessful && response.body() != null) {
                        // Cập nhật local database với thông tin từ server
                        val serverSubtask = response.body()!!
                        val updatedEntity = serverSubtask.toEntity(subtaskEntity).copy(
                            syncStatus = PersonalTask.SyncStatus.SYNCED,
                            lastModified = System.currentTimeMillis()
                        )
                        personalTaskDao.updateSubtask(updatedEntity)

                        return Result.success(updatedEntity.toDomainModel())
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Error syncing subtask to server", e)
                }
            }

            return Result.success(subtaskEntity.toDomainModel())
        } catch (e: Exception) {
    ...
    override suspend fun updateSubtask(subtask: Subtask): Result<Subtask> {
        try {
            ...
            personalTaskDao.updateSubtask(subtaskEntity)

            // Nếu có kết nối mạng và subtask đã có serverId, đồng bộ lên server
            if (connectionChecker.isNetworkAvailable() && subtask.serverId != null) {
                try {
                    // Gọi API để cập nhật subtask trên server
                    val request = subtask.toApiRequest()
                    val response = apiService.updateSubtask(subtask.serverId, request)
                    ...
                    Log.e(TAG, "Error syncing subtask update to server", e)
                }
            }
    ...
    override suspend fun deleteSubtask(subtaskId: String): Result<Unit> {
        try {
            val subtask = personalTaskDao.getSubtaskById(subtaskId)
            if (subtask != null) {
                if (subtask.serverId != null) {
                    // Nếu subtask đã được đồng bộ với server, đánh dấu để xóa sau
                    personalTaskDao.markSubtaskForDeletion(subtaskId, System.currentTimeMillis())

                    // Nếu có kết nối mạng, xóa trên server
                    if (connectionChecker.isNetworkAvailable()) {
                        try {
                            // Gọi API để xóa subtask trên server
                            val response = apiService.deleteSubtask(subtask.serverId)

                            if (response.isSuccessful) {
                                // Xóa subtask khỏi local database
                                personalTaskDao.deleteSubtask(subtaskId)
                            }
                        } catch (e: Exception) {
                            Log.e(TAG, "Error syncing subtask deletion to server", e)
                        }
                    }
                } else {
                    // Nếu subtask chưa được đồng bộ với server, xóa luôn
                    personalTaskDao.deleteLocalOnlySubtask(subtaskId)
                }
            }
    ...
    override suspend fun updateTaskOrder(taskId: String, newOrder: Int): Result<Unit> {
        try {
            ...
            personalTaskDao.updateTaskOrder(taskId, newOrder, PersonalTask.SyncStatus.UPDATED, System.currentTimeMillis())
            ...
            if (connectionChecker.isNetworkAvailable() && task?.serverId != null) {
                try {
                    ...
                    val response = apiService.updateTaskOrder(task.serverId, newOrder)

                    if (response.isSuccessful) {
                        // Cập nhật trạng thái đồng bộ
                        personalTaskDao.updateTaskSyncStatus(taskId, PersonalTask.SyncStatus.SYNCED, System.currentTimeMillis())
                    }
                } catch (e: Exception) {
    ...
    override suspend fun updateTasksOrder(taskOrders: Map<String, Int>): Result<Unit> {
        try {
            ...
            for ((taskId, order) in taskOrders) {
                personalTaskDao.updateTaskOrder(taskId, order, PersonalTask.SyncStatus.UPDATED, System.currentTimeMillis())
            }
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    val tasks = personalTaskDao.getTasksByIds(taskOrders.keys.toList())
                    val serverTaskOrders = tasks
                        .filter { it.serverId != null }
                        .associate { it.serverId!! to taskOrders[it.id]!! }

                    if (serverTaskOrders.isNotEmpty()) {
                        // Gọi API để cập nhật thứ tự tasks trên server
                        val response = apiService.updateTasksOrder(serverTaskOrders)

                        if (response.isSuccessful) {
                            // Cập nhật trạng thái đồng bộ
                            for (taskId in taskOrders.keys) {
                                personalTaskDao.updateTaskSyncStatus(taskId, PersonalTask.SyncStatus.SYNCED, System.currentTimeMillis())
                            }
                        }
                    }
    ...
    override suspend fun updateSubtaskOrder(subtaskId: String, newOrder: Int): Result<Unit> {
        try {
            // Cập nhật thứ tự trong local database
            personalTaskDao.updateSubtaskOrder(subtaskId, newOrder, PersonalTask.SyncStatus.UPDATED, System.currentTimeMillis())

            // Nếu có kết nối mạng, đồng bộ lên server
            val subtask = personalTaskDao.getSubtaskById(subtaskId)
            if (connectionChecker.isNetworkAvailable() && subtask?.serverId != null) {
                try {
                    // Gọi API để cập nhật thứ tự subtask trên server
                    val response = apiService.updateSubtaskOrder(subtask.serverId, newOrder)

                    if (response.isSuccessful) {
                        // Cập nhật trạng thái đồng bộ
                        personalTaskDao.updateSubtaskSyncStatus(subtaskId, PersonalTask.SyncStatus.SYNCED, System.currentTimeMillis())
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Error syncing subtask order to server", e)
                }
            }

            return Result.success(Unit)
        } catch (e: Exception) {
            Log.e(TAG, "Error updating subtask order", e)
            return Result.failure(e)
        }
    }
    ...
    override suspend fun updateSubtasksOrder(subtaskOrders: Map<String, Int>): Result<Unit> {
        try {
            // Cập nhật thứ tự trong local database
            for ((subtaskId, order) in subtaskOrders) {
                personalTaskDao.updateSubtaskOrder(subtaskId, order, PersonalTask.SyncStatus.UPDATED, System.currentTimeMillis())
            }

            // Nếu có kết nối mạng, đồng bộ lên server
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    // Lấy danh sách subtask có serverId
                    val subtasks = personalTaskDao.getSubtasksByIds(subtaskOrders.keys.toList())
                    val serverSubtaskOrders = subtasks
                        .filter { it.serverId != null }
                        .associate { it.serverId!! to subtaskOrders[it.id]!! }

                    if (serverSubtaskOrders.isNotEmpty()) {
                        // Gọi API để cập nhật thứ tự subtasks trên server
                        val response = apiService.updateSubtasksOrder(serverSubtaskOrders)

                        if (response.isSuccessful) {
                            // Cập nhật trạng thái đồng bộ
                            for (subtaskId in subtaskOrders.keys) {
                                personalTaskDao.updateSubtaskSyncStatus(subtaskId, PersonalTask.SyncStatus.SYNCED, System.currentTimeMillis())
                            }
                        }
                    }
                } catch (e: Exception) {
                    Log.e(TAG, "Error syncing subtasks order to server", e)
                }
            }

            return Result.success(Unit)
        } catch (e: Exception) {
            Log.e(TAG, "Error updating subtasks order", e)
            return Result.failure(e)
        }
    }

    override suspend fun batchUpdateTasks(tasks: List<PersonalTask>): Result<List<PersonalTask>> {
        return try {
            if (!connectionChecker.isNetworkAvailable()) {
                return Result.failure(IOException("No network connection"))
            }

            val requests = tasks.map { it.toApiRequest() }
            val response = apiService.batchUpdatePersonalTasks(requests)

            if (response.isSuccessful && response.body() != null) {
                val updatedTasks = response.body()!!.map { serverTask ->
                    // Cập nhật local database
                    val existingTask = personalTaskDao.getTaskById(serverTask.id)
                    val updatedEntity = if (existingTask != null) {
                        serverTask.toEntity(existingTask).copy(syncStatus = "synced")
                    } else {
                        serverTask.toEntity().copy(syncStatus = "synced")
                    }
                    personalTaskDao.updateTask(updatedEntity)
                ...
                val errorBody = response.errorBody()?.string() ?: "Unknown error"
                Log.e(TAG, "Error batch updating tasks: $errorBody")
                Result.failure(Exception(errorBody))
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error batch updating tasks", e)
            Result.failure(e)
        }
    }
    ...
    override suspend fun filterTasksFromServer(
        ...
        return try {
            ...
            val response = apiService.filterPersonalTasks(
                status = status,
                priority = priority,
