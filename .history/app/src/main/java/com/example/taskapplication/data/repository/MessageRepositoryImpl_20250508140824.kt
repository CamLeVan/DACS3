...
class MessageRepositoryImpl @Inject constructor(
    ...
    override fun getMessageById(messageId: String): Flow<Message?> {
        ...
        return try {
            if (networkUtils.isNetworkAvailable()) {
                ...
                if (response.isSuccessful) {
                    ...
                    messageReactionDao.insertReaction(messageReaction)
                    Result.success(Unit)
                } else {
    ...
    override suspend fun removeReaction(messageId: String, userId: String, reaction: String): Result<Unit> {
        return try {
            if (networkUtils.isNetworkAvailable()) {
                ...
                if (response.isSuccessful) {
                    messageReactionDao.deleteReaction(messageId, userId, reaction)
                    Result.success(Unit)
                } else {
                    messageReactionDao.markReactionForDeletion(messageId, userId, reaction, System.currentTimeMillis())
                    Result.success(Unit)
                }
            } else {
                messageReactionDao.markReactionForDeletion(messageId, userId, reaction, System.currentTimeMillis())
    ...
    override suspend fun syncMessages(): Result<Unit> {
        return try {
            ...
            for (readStatus in pendingReadStatuses) {
                ...
                if (response.isSuccessful) {
                    messageReadStatusDao.markReadStatusAsSynced(readStatus.messageId, readStatus.userId)
                }
            }
            ...
            for (reaction in pendingReactions) {
                when (reaction.syncStatus) {
                    "pending_create" -> {
                        ...
                        if (response.isSuccessful) {
                            messageReactionDao.markReactionAsSynced(reaction.messageId, reaction.userId, reaction.reaction)
                        }
                    }
                    "pending_delete" -> {
                        val response = apiService.removeReaction(reaction.messageId, reaction.userId, reaction.reaction)
                        if (response.isSuccessful) {
                            messageReactionDao.deleteReaction(reaction.messageId, reaction.userId, reaction.reaction)
                        }
                    }
                }
            }
