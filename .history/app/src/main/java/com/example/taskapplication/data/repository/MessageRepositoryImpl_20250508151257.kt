...
class MessageRepositoryImpl @Inject constructor(
    ...
    private suspend fun createMessage(message: Message): Result<Message> {
        return try {
    ...
    override suspend fun removeReaction(messageId: String, reactionId: String): Result<Unit> {
        return try {
            val userId = dataStoreManager.getCurrentUserId()
            if (networkUtils.isNetworkAvailable()) {
                val response = apiService.removeReaction(messageId, userId, reactionId)
                if (response.isSuccessful) {
                    messageReactionDao.deleteReaction(reactionId)
                    Result.success(Unit)
                } else {
                    messageReactionDao.markReactionForDeletion(reactionId, System.currentTimeMillis())
                    Result.success(Unit)
                }
            } else {
                messageReactionDao.markReactionForDeletion(reactionId, System.currentTimeMillis())
    ...
    override suspend fun getUnreadMessageCount(): Result<Map<String, Int>> {
        return try {
            val userId = dataStoreManager.getCurrentUserId()
            val teams = teamDao.getTeamsByUserSync(userId)
            val unreadCounts = mutableMapOf<String, Int>()
            
            for (team in teams) {
                val count = messageDao.getUnreadMessagesCount(team.id, userId).first()
                unreadCounts[team.id] = count
            }
            
            Result.success(unreadCounts)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
