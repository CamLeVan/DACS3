...
fun DocumentFolderDto.toDomain(): DocumentFolder {
    return DocumentFolder(
        ...
        createdBy = createdBy,
        createdAt = parseApiDateToLong(createdAt),
        updatedAt = parseApiDateToLong(updatedAt),
        syncStatus = "synced",
        serverId = id
    )
}
...
fun DocumentDto.toDomain(): Document {
    return Document(
        ...
        uploadedAt = parseApiDateToLong(uploadedAt),
        lastModified = parseApiDateToLong(lastModified),
...
fun DocumentPermissionDto.toDomain(): DocumentPermission {
    return DocumentPermission(
        ...
        grantedBy = grantedBy,
        grantedAt = parseApiDateToLong(grantedAt),
        syncStatus = "synced",
        serverId = id
    )
}
...
/**
 * Helper function to parse API date string to Long
 */
private fun parseApiDateToLong(dateString: String): Long {
    // This is a simplified implementation. In a real app, you would use a proper date parser
    return try {
        // Assuming ISO 8601 format: "2023-01-01T12:00:00Z"
        val date = java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'").parse(dateString)
