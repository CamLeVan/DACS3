    ...
    override suspend fun syncDocuments(): Resource<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val pendingCreateDocuments = documentDao.getUnSyncedDocuments().filter { it.syncStatus == "pending_create" }
                for (document in pendingCreateDocuments) {
                    try {
                        // Tạo tài liệu trên server
                        val documentDomain = document.toDomain()
                        val file = File(context.filesDir, "documents/${document.id}")

                        if (file.exists()) {
                            // Tạo request để tải lên server
                            val requestBody = file.asRequestBody("multipart/form-data".toMediaTypeOrNull())
                            val filePart = MultipartBody.Part.createFormData("file", file.name, requestBody)
                        ...
                        // Ghi log lỗi và tiếp tục với tài liệu tiếp theo
                        e.printStackTrace()
                    }
                }

                // Đồng bộ các tài liệu đang chờ cập nhật
                val pendingUpdateDocuments = documentDao.getUnSyncedDocuments().filter { it.syncStatus == "pending_update" }
                for (document in pendingUpdateDocuments) {
                    try {
                        ...
                        documentDao.updateDocumentSyncStatus(document.id, "synced", document.serverId)
                    } catch (e: Exception) {
                        // Ghi log lỗi và tiếp tục với tài liệu tiếp theo
                        e.printStackTrace()
                    }
                ...
                for (version in pendingCreateVersions) {
                    try {
                        ...
                        if (document != null) {
                            // Tìm file phiên bản
                            val file = File(context.filesDir, "documents/${version.documentId}_v${version.versionNumber}")
                        ...
                        // Ghi log lỗi và tiếp tục với phiên bản tiếp theo
                        e.printStackTrace()
                    }
                }
                ...
                for (permission in pendingCreatePermissions) {
                    try {
                        // Lấy thông tin tài liệu
                        val document = documentDao.getDocumentById(permission.documentId)
                        if (document != null) {
