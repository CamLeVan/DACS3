...
class NotificationRepositoryImpl @Inject constructor(
    ...
    override suspend fun getNotifications(since: Long?): Result<List<Notification>> {
        return try {
            if (networkUtils.isNetworkAvailable()) {
                ...
                val entities = notificationDao.getNotificationsSync()
                Result.success(entities.map { it.toDomainModel() })
            }
        } catch (e: Exception) {
            Result.failure(e)
    ...
    override suspend fun registerDevice(deviceId: String, fcmToken: String): Result<Unit> {
        return try {
            if (networkUtils.isNetworkAvailable()) {
                val response = apiService.registerDevice(deviceId, fcmToken)
                if (response.isSuccessful) {
                    Result.success(Unit)
                } else {
        ...
        return try {
            if (networkUtils.isNetworkAvailable()) {
                ...
                if (response.isSuccessful) {
                    ...
                    Result.failure(Exception("Failed to unregister device: ${response.code()}"))
                }
            } else {
                Result.failure(Exception("No network connection"))
            }
