...
import com.example.taskapplication.data.util.NetworkUtils
...
class TeamDocumentRepositoryImpl @Inject constructor(
    ...
    override fun getDocumentsByTeam(teamId: String): Flow<List<TeamDocument>> {
        return teamDocumentDao.getDocumentsByTeam(teamId)
    ...
    override suspend fun getDocumentById(documentId: String): TeamDocument? {
        return withContext(Dispatchers.IO) {
            teamDocumentDao.getDocumentById(documentId)?.toDomainModel()
        }
    }

    override fun getDocumentsByUser(userId: String): Flow<List<TeamDocument>> {
        return teamDocumentDao.getDocumentsByUser(userId)
            .map { entities -> entities.map { it.toDomainModel() } }
            .catch { e ->
                // Log error and emit empty list
                emit(emptyList())
    ...
    override fun getDocumentsByAccessLevel(teamId: String, accessLevel: String): Flow<List<TeamDocument>> {
        return teamDocumentDao.getDocumentsByAccessLevel(teamId, accessLevel)
            .map { entities -> entities.map { it.toDomainModel() } }
            .catch { e ->
                // Log error and emit empty list
                emit(emptyList())
    ...
    override fun getDocumentsByFileType(teamId: String, fileType: String): Flow<List<TeamDocument>> {
        return teamDocumentDao.getDocumentsByFileType(teamId, fileType)
            .map { entities -> entities.map { it.toDomainModel() } }
            .catch { e ->
                // Log error and emit empty list
                emit(emptyList())
    ...
    override fun searchDocuments(teamId: String, query: String): Flow<List<TeamDocument>> {
        return teamDocumentDao.searchDocuments(teamId, query)
            .map { entities -> entities.map { it.toDomainModel() } }
            .catch { e ->
                // Log error and emit empty list
                emit(emptyList())
    ...
    override suspend fun uploadDocument(
        ...
        return withContext(Dispatchers.IO) {
            try {
                val userId = dataStoreManager.getCurrentUserId() ?: return@withContext Result.failure(
                ...
                if (networkUtils.isNetworkAvailable()) {
                    try {
                        ...
                        teamDocumentDao.updateDocumentSyncStatus(documentId, "synced", "server_id_placeholder")
                    } catch (e: Exception) {
    ...
    override suspend fun deleteDocument(documentId: String): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                teamDocumentDao.markDocumentAsDeleted(documentId)
    ...
    override suspend fun downloadDocument(documentId: String): Result<File> {
        return withContext(Dispatchers.IO) {
            try {
                val document = teamDocumentDao.getDocumentById(documentId) ?: return@withContext Result.failure(
