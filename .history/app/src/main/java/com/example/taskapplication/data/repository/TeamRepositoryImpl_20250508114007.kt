...
class TeamRepositoryImpl @Inject constructor(
    ...
    override fun getTeamsByUser(userId: String): Flow<List<Team>> = flow {
        val teams = teamDao.getTeamsByUser(userId.toLong()).map { it.toDomainModel() }
        emit(teams)
    }
    ...
    override suspend fun syncTeamsByUser(userId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            val pendingTeams = teamDao.getPendingTeamsByUserSync(userId.toLong())
            for (team in pendingTeams) {
                when (team.syncStatus) {
                    "pending" -> {
                        val response = apiService.createTeam(team.toApiRequest())
    ...
    override suspend fun syncTeamMembers(teamId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            val pendingMembers = teamDao.getPendingMembersSync(teamId.toLong())
            for (member in pendingMembers) {
                when (member.syncStatus) {
                    "pending" -> {
                        val response = apiService.createTeamMember(teamId, member.toApiRequest())
