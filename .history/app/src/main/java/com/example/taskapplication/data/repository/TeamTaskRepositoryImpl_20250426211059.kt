...
class TeamTaskRepositoryImpl @Inject constructor(
    ...
    override fun getTeamTasksForTeam(teamId: String): Flow<List<TeamTask>> {
        return teamTaskDao.getTeamTasksByTeamId(teamId).map { entities ->
            entities.map { it.toDomainModel() }
        }
    }

    override fun getTeamTaskById(taskId: String): Flow<TeamTask?> {
        return teamTaskDao.getTeamTaskById(taskId).map { it?.toDomainModel() }
    }
    ...
    override suspend fun createTeamTask(task: TeamTask): Result<TeamTask> {
        ...
        teamTaskDao.insertTeamTask(taskWithId.toEntity())
    ...
    override suspend fun updateTeamTask(task: TeamTask): Result<TeamTask> {
        ...
        teamTaskDao.updateTeamTask(updatedTask.toEntity())
    ...
    override suspend fun getAssignedTeamTasks(userId: String): Flow<List<TeamTask>> {
        return teamTaskDao.getTeamTasksByAssignedTo(userId).map { entities ->
            entities.map { it.toDomainModel() }
        }
    }

    override suspend fun getCreatedTeamTasks(userId: String): Flow<List<TeamTask>> {
        return teamTaskDao.getTeamTasksByCreatedBy(userId).map { entities ->
            entities.map { it.toDomainModel() }
        }
    }
        ...
        try {
            ...
            for (task in tasksToCreate) {
                try {
                    ...
                    val taskRequest = task.toApiRequest().copy(teamId = team.serverId)
                    val response = apiService.createTeamTask(taskRequest)
            ...
            for (task in tasksToUpdate) {
                try {
                    ...
                    // Use server id for the team
                    val taskRequest = task.toApiRequest().copy(teamId = team.serverId)
                    val response = apiService.updateTeamTask(task.serverId, taskRequest)
