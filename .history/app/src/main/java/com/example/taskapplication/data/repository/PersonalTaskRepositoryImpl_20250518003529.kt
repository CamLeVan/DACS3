...
class PersonalTaskRepositoryImpl @Inject constructor(
        ...
        for (task in pendingUpdateTasks) {
            if (task.serverId != null) {
                try {
                    ...
                    // Xử lý lỗi IllegalStateException (JSON parsing error)
                    if (e is IllegalStateException && e.message?.contains("Expected BEGIN_OBJECT but was BEGIN_ARRAY") == true) {
                        Log.e(TAG, "JSON parsing error: Server returned array instead of object. Marking task as synced to prevent further errors.")
    ...
    private suspend fun syncServerChangesToLocal() {
        try {
                ...
                if (meta != null && meta.currentPage < meta.lastPage) {
                    ...
                    for (page in meta.currentPage + 1..meta.lastPage) {
                        try {
                            ...
                            // Xử lý lỗi IllegalStateException (JSON parsing error)
                            if (e is IllegalStateException && e.message?.contains("Expected BEGIN_OBJECT but was BEGIN_ARRAY") == true) {
                                Log.e(TAG, "JSON parsing error: Server returned array instead of object. Skipping next pages.")
                                break // Dừng việc lấy các trang tiếp theo
                            }
                        }
                    }
            ...
            // Xử lý lỗi IllegalStateException (JSON parsing error)
            if (e is IllegalStateException && e.message?.contains("Expected BEGIN_OBJECT but was BEGIN_ARRAY") == true) {
                Log.e(TAG, "JSON parsing error: Server returned array instead of object. Using local data only.")
