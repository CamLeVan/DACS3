...
class MessageRepositoryImpl @Inject constructor(
    ...
    override suspend fun sendMessage(message: Message): Result<Message> {
        if (message.teamId == null && message.taskId == null) {
            return Result.failure(IllegalArgumentException("Message must have either teamId or taskId"))
        }
    ...
    override suspend fun syncMessages(): Result<Unit> {
        ...
        try {
            ...
            for (message in messagesToCreate) {
                try {
                    val response = if (message.teamId != null) {
                        apiService.sendTeamMessage(message.toApiRequest())
                    } else {
                        apiService.sendTaskMessage(message.toApiRequest())
                    }
