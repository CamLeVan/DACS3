...
interface TeamDocumentDao {
    ...
    suspend fun markDocumentAsDeleted(documentId: Long, timestamp: Long = System.currentTimeMillis())
    ...
    suspend fun updateDocumentSyncStatus(documentId: Long, syncStatus: String, serverId: Long?, timestamp: Long = System.currentTimeMillis())
    ...
    suspend fun updateDocumentAccessLevel(documentId: Long, accessLevel: String, allowedUsers: String, timestamp: Long = System.currentTimeMillis())

    /**
     * Get documents by folder
     */
    @Query("SELECT * FROM team_documents WHERE folderId = :folderId AND isDeleted = 0 ORDER BY name ASC")
