...
import com.example.taskapplication.data.api.request.UnregisterDeviceRequest
...
class NotificationRepositoryImpl @Inject constructor(
    ...
    override suspend fun unregisterDevice(deviceId: String): Result<Unit> {
        return try {
            val request = UnregisterDeviceRequest(deviceId = deviceId)
            val response = apiService.unregisterDevice(request)
